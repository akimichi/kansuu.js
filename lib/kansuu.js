...
module.exports = {
  ...
  isFunction: (target) => {
    var self = this;
    return self.typeOf(target) === 'function';
  },
  ...
  stream: {
    ...
    isStream: (target) => {
      var self = this;
      if(self.isFunction(target)) {
        var obj = self.stream.force.call(self,target);
        return [self.equal(self.plucker('type')(obj))('stream'),
                self.truthy(self.plucker('value')(obj)),
                self.truthy(self.plucker('next')(obj))].reduce((accumulator,item) => {
    ...
    flatten: (stream) => {
      ...
      if(self.stream.isEmpty.call(self,stream)){
        ...
        // if(self.stream.isStream.call(self,head)) {
        //   return self.stream.append.call(self,head)(self.stream.flatten.call(self,tail));
        // } else {
        //   return self.stream.cons.call(self,head)(self.stream.flatten.call(self,tail));
        // }
      }
          ...
          if(self.truthy(predicate(value))){
            ...
            return self.stream.delay.call(self,(_) => {
              return {
                type : 'stream',
                value : value,
                next : self.stream.filter.call(self,next)(predicate)
              };
            });
