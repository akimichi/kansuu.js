...
module.exports = {
	...
	return function(n){
	  ...
	  if (n === 0)
		...
		if(self.isEmpty(array)) {
		  ...
		  var head = self.list.head(array);
		  var tail = self.list.tail(array);
		  return self.list.cons(head)(self.take.bind(self)(tail)(n-1));
		}
	  }
	};
  },
  ...
  list: {
	  ...
	  var take = function(list){
		return function(n) {
		  ...
		  if (n === 0) {
			...
			return [list.head].concat(take.bind(self)(list.tail())(n-1));
		  }
		};
	  };
	  return function(tail){
		expect(tail).to.an('array');
		var list = {
		  type : 'list',
		  head : head,
		  tail : function(_){
			...
			return self.list.mkList(tail[0])(tail.slice(1,tail.length));
		  },
		  take : function(n){
			return take.bind(self)(list)(n);
		  }
		};
		Object.freeze(list);
		return list;
	...
	init: function(list){
	  var self = this;
	  self.list.censor(list);
	  return self.list.mkList(self.list.take(list)(list.length() - 1));
	},
	  ...
	  var list = {
		...
		tail: function(_){
		  ...
		  if(array[1] == undefined){
			...
			return self.list.mkList(array.slice(1,array.length));
		  }
		},
		take: function(n){
		  var self = this;
		  return self.list.take.bind(self)(list)(n);
		},
		reduce: function(accumulator){
		  return function(glue){
			return self.list.reduce.bind(self)(list)(accumulator)(glue);
		  };
		},
		length : function(_){
		...
		// ~~~
		init: function(_){
		  return self.list.init(list);
		},
		// reverse: function(_){
		//   return reduce.bind(self)(list)(self.list.nil)(function(item) {
		// 	return function(accumulator){
		// 	  return self.list.mkList([item].concat(accumulator.tail()));
		// 	};
		//   });
		// },
	  };
