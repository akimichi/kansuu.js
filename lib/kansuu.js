...
module.exports = {
  ...
  monad: {
	...
	random: {
	  ...
	  int: (rng) => {
        var self = this;
		var random = Math.floor(rng() * 10);
		//var random = Math.floor(rng() * Number.MAX_VALUE);
		var newRng = seedrandom(random);
		return self.monad.random.unit.bind(self)(random)(newRng);
	  },
	  // random.ints
	  //
