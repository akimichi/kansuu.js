...
module.exports = {
  ...
  monad: {
    ...
    maybeMonad: {
      ...
      unit: (value) => {
        ...
        if(self.existy(value)){
          ...
          return self.monad.maybeMonad.nothing.bind(self);
        }
      },
      // isEqual: (maybeA) => {
      //   var self = this;
      ...
      isEqual: (maybeA) => {
        var self = this;
        return (maybeB) => {
          return self.algebraic.match(maybeA,{
            just: (valueA) => {
              return self.algebraic.match(maybeB,{
                just: (valueB) => {
            ...
            nothing: self.algebraic.match(maybeB,{
              just: (_) => {
                return false;
              },
              nothing: true
    ...
    stream:{
      ...
      head: (lazyList) => {
        var self = this;
        return self.algebraic.match(lazyList,{
          empty: (_) => {
			return self.monad.maybeMonad.nothing;
		  },
          cons: (value, tailThunk) => {
            return self.monad.maybeMonad.just.call(self,value);
          }
        });
      },
      ...
      tail: (lazyList) => {
        ...
        return self.algebraic.match(lazyList,{
          ...
          cons: (head, tailThunk) => {
            return self.monad.maybeMonad.just.call(self,
												   tailThunk());
		  }
        });
      },
	  isEmpty: (stream) => {
        var self = this;
		return self.algebraic.match(stream,{
          empty: (_) => {
            return true;
          },
      ...
      append: (stream1, stream2) => {
		var self = this;
		return self.algebraic.match(stream1,{
		  empty: (_) => {
            return stream2;
		  },
		  cons: (head1,tailThunk1) => {
			return self.algebraic.match(stream2,{
			  empty: (_) => {
				return stream1;
			  },
      ...
      concat: (xs) => {
		var self = this;
		return (ys) => {
		  return self.algebraic.match(xs,{
			empty: (_) => {
			  return ys;
			},
			cons: (head,tailThunk) => {
      ...
      flatten: (lazyList) => {
		...
		return self.algebraic.match(lazyList,{
          ...
          cons: (head,tailThunk) => {
			return self.monad.stream.concat.call(self,
												 head)(self.monad.stream.flatten.call(self,
																					  tailThunk()));
		  }
		});
	  },
      ...
      //     cons: (head,tailThunk) => {
	  // 		return self.monad.stream.append.call(self,
	  // 											 self.monad.stream.unit.call(self,head),
	  // 											 self.monad.stream.flatten.call(self,tailThunk()));
	  // 	  }
      //   });
      // },
	  // ### stream#flatMap
