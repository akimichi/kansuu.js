...
module.exports = {
  ...
  list: {
	...
	// empty: function(){
    //   var self = this;
	//   return self.list.cons.bind(self)(base.nothing)(base.fail);
	// }(),
...
//	  if(list.head === base.nothing){
    ...
    toArray: function(list){
      ...
      if(self.list.isEmpty(list)){
        ...
        return self.cons.bind(self)(list.head)(self.list.toArray.bind(self)(list.tail.bind(self)()));
      }
    },
    // take :: List => List
    take: function(list){
      ...
      return function(n) {
        ...
        if (n === 0) {
          ...
          return self.list.cons.bind(self)(list.head)(self.list.take.bind(self)(list.tail.bind(self)())(n-1));
    ...
    merge: function(listX) {
      ...
      return function(listY){
        ...
        if(self.list.isEmpty(listX)){
          return listY;
        }
        if(self.list.isEmpty(listY)){
          return listX;
        }
        var x = listX.head;
        var xs = listX.tail();
  ...
  stream: {
    ...
    cons: function(any){
      ...
      return function(subsequent){
        ...
        var obj = {
          ...
          value: any,
          next : function(){
            return self.stream.cons.bind(self)(subsequent(any))(subsequent);
          }
        };
        Object.freeze(obj);
        return obj;
	...
	// mkStream: function(array){
    //   expect(array).to.an('array');
    //   var self = this;
    //   if(array.length === 1){
    //     return self.stream.cons.bind(self)(self.head(array))(base.thunk(base.nothing));
    //   } else {
    //     return self.stream.cons.bind(self)(self.head(array))(base.thunk(self.stream.mkStream.bind(self)(self.tail(array))));
    //   }
	// },
	mkStream: function(init){
      var self = this;
      return function(subsequent){
		return self.stream.cons.bind(self)(init)(subsequent);
      };
    },
	// stream.empty
	empty: {
	...
	// empty: function(){
    //   var self = this;
	//   return self.stream.cons.bind(self)(base.nothing)(base.fail);
	// },
	isEmpty: function(stream){
    ...
    repeat: function(any){
      var self = this;
      return self.stream.cons.bind(self)(any)(self.id);
    },
	// stream.reduce
    reduce: function(stream){
      var self = this;
      self.list.censor(stream);
      return function(accumulator){
        return function(glue){
          expect(glue).to.a('function');
          if(self.stream.isEmpty(stream)) {
			...
			var next = stream.next.bind(self)();
            return glue(item)(self.stream.reduce.bind(self)(next)(accumulator)(glue));
          }
        };
      };
    },
	exists: function(stream){
      var self = this;
      self.list.censor(stream);
      return function(predicate){
		return self.stream.reduce(stream)(false)(function(item){
		  return function(accumulator){
