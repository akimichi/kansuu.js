...
const join = (xss) => {
  ...
  return reduce(xss)([])(__.curry(concat));
};
...
// and = foldr (&&) false
const and = (anArray) => {
  return reduce(anArray)(true)(__.and);
};
...
// or = foldr (||) false
const or = (anArray) => {
  return reduce(anArray)(false)(__.or);
};
...
const all = (anArray) => {
  return (predicate) => {
    return __.compose(and, __.flip(map)(predicate))(anArray)
  };
};
...
const any = (anArray) => {
  return (predicate) => {
    return __.compose(or, __.flip(map)(predicate))(anArray)
  };
};
  ...
  return (accumulator) => {
    return (transform) => {
      ...
      if(isEmpty(anArrray)){
        ...
        return foldl(tail(anArrray))(transform(head(anArrray))(accumulator))(transform);
      }
    };
  };
};
...
// ~~~
const foldl1 = (anArrray) => {
  expect(anArrray).to.not.be.empty();
  return (transform) => {
    expect(transform).to.a('function');
    return foldl(tail(anArrray))(head(anArrray))(transform);
  };
};
...
const flatten = (anArrayOfArray) => {
  if(isEmpty(anArrayOfArray) === true) {
    return [];
  } else {
    return concat(head(anArrayOfArray), flatten(tail(anArrayOfArray)));
  }
  // empty: (_) => {
