...
const __ = require('../lib/kansuu.js'),
  ...
  Vector = require('./kansuu-array.js'),
...
const Parser = {
  ...
  lineComment: (prefix) => {
    ...
    return Parser.flatMap(Parser.chars(List.fromString(prefix)))(_ => {
  ...
  identifier: (keywords) => {
    ...
    // const reserved = Vector.reduce(keywords)([])(keyword => {
    //   expect(keyword).to.a('string');
    //   return (accumulator) => {
    //     return Vector.cons(keyword, accumulator);
    //   };
    // });
    // expect(reserved).to.a('array');
    return Parser.token(Parser.flatMap(Parser.ident())(x => {
      console.log(List.toString(x))
      // return Parser.unit(x);
