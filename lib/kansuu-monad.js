...
const Parser = {
  ...
  append: (parser1) => {
    return (parser2) => {
      return (input) => {
        return List.append(Parser.parse(parser1)(input))(Parser.parse(parser2)(input));
      };
    };
  },
  ...
  many: (p) => {
    return Parser.alt(Parser.some(p),Parser.unit(List.empty()));
    // NG!
    // return List.append(
    //   Parser.flatMap(p)(x => {
    ...
    //     });
    //   })
    // )(List.cons(List.empty(),List.empty()));
  },
  // ## Parser#many1
  ...
  some: (x) => {
    return Parser.flatMap(x)((a) => {
      return Parser.flatMap(Parser.many(x))((b) => {
        return Parser.unit(List.cons(a,b));
      });
    }); 
  },
  // ## Parser#alphanum
  ...
  word: () => {
    ...
    return Parser.append(neWord())(Parser.unit(List.empty()));
  ...
  ident: () => {
    return Parser.flatMap(Parser.lower())(x => {
      return Parser.flatMap(Parser.many(Parser.alphanum()))(xs => {
        return Parser.unit(List.cons(x,xs));
        // return Parser.unit(Symbol(List.toString(List.cons(x,xs))));
      });
    });
  },
  // ## Parser#nat
  ...
  string: () => {
    ...
    const stringContent = () => {
      ...
      return Parser.flatMap(Parser.many(Parser.sat(notQuote)))((xs) => {
        return Parser.unit(List.toString(xs));
      });
    };
    return Parser.token(
  ...
  sepBy1: (parser) => {
    return (sep) => {
      return Parser.flatMap(parser)(x => {
        return Parser.flatMap(Parser.many(
          Parser.flatMap(sep)(_ => {
  ...
  sepBy: (parser) => {
    return (sep) => {
      return Parser.append(Parser.sepBy1(parser)(sep))(List.cons(List.empty(),List.empty()));
    };
  },
  // ## Parser#bracket
  // ~~~haskell
  ...
  chainl1: (parser, op) => {
    // const rest = (x) => {
    //   return Parser.flatMap(op)(f => {
    //     return Parser.flatMap(p)(y => {
    //       return rest(f(x)(y))
    //     });
    //   });
    // };
    // return Parser.flatMap(p)(rest);
    return Parser.flatMap(parser)(x => { // x <- p
      // fys <- many [(f,y) | f <- op, y <- p]]
      return Parser.flatMap(Parser.many(Parser.flatMap(op)(f => {
        return Parser.flatMap(parser)(y => {
