      ...
      evaluate: (exp) => {
		return (environment) => {
          return self.match(exp, {
			...
			variable: (name) => {
              return self.ordinary.unit(self.env.lookupEnv(name, environment));
			},
			number: (n) => {
			...
			lambda: (identifier, bodyExp) => {
              ...
              return (actualArg) => {
				return self.match(identifier,{ // maybeを返すべきか？
                  variable: (name) => {
					return self.ordinary.evaluate(bodyExp)(self.env.extendEnv(name, actualArg ,environment));
                  }
				});
              };
			},
			app: (exp, arg) => {
			  return self.ordinary.flatMap(self.ordinary.evaluate(exp)(environment))((rator) => {
				return self.ordinary.flatMap(self.ordinary.evaluate(arg)(environment))((rand) => {
